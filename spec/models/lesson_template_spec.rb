...
describe LessonTemplate do
  ...
  describe "end_at_hour validation" do
    subject { FactoryBot.build(:lesson_template) }
  ...
  describe '#located' do
    context "Lesson are located in moliere" do
      before { FactoryBot.create(:lesson_template) }

      it "Must find the lesson" do
        expect(LessonTemplate.located('moliere').count).to eql 1
      end

      it "Must not find the lesson" do
        expect(LessonTemplate.located('maillot').count).to eql 0
      end

      it "Must not find the lesson" do
        expect(LessonTemplate.located('amelot').count).to eql 0
      end
    end
  end
end
